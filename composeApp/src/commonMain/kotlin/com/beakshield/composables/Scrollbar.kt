...
@Composable
fun Modifier.beakshieldLazyScrollbar(
    state: LazyListState,
    horizontal: Boolean = false,
    knobColor: Color = mainColor,
    visibleAlpha: Float = 1f,
    hiddenAlpha: Float = 0.5f,
): Modifier {
    return this.lazyScrollbar(
        state = state,
        horizontal = horizontal,
        knobColor = knobColor,
        trackColor = secondaryGoldColor,
...
@Composable
fun Modifier.beakshieldScrollbar(
    state: ScrollState,
    horizontal: Boolean = false,
    knobColor: Color = mainColor,
    visibleAlpha: Float = 1f,
...
/**
 * LAZY LIST SCROLLBAR ORIGINAL DEVELOPER @Warlax (04-19-22) (https://stackoverflow.com/a/71932181)
 * REGULAR LIST HEAVILY USED ORIGINAL DEVELOPER CODE
 * Renders a scrollbar.
 *
...
fun Modifier.lazyScrollbar(
    ...
    return drawWithContent {
        ...
        state.layoutInfo.visibleItemsInfo.firstOrNull()?.let { firstVisibleItem ->
            // Hide scrollbar if everything fits in the viewport
            if (state.layoutInfo.visibleItemsInfo.size >= state.layoutInfo.totalItemsCount) return@drawWithContent
            ...
            if (state.isScrollInProgress || alpha > 0f) {
                ...
                // this scrollbar.
                val viewportSize = (if (horizontal) size.width else size.height) - padding.toPx() * 2
                ...
                drawRoundRect(
                    ...
                    size = if (horizontal) Size(knobSize, thickness.toPx()) else Size(thickness.toPx(), knobSize),
                    alpha = alpha,
...
fun Modifier.scrollbar(
    ...
    return drawWithContent {
        ...
        if (state.isScrollInProgress || alpha > 0f) {
            val viewportSize = (if (horizontal) size.width else size.height) - padding.toPx() * 2
            val contentSize = state.maxValue.toFloat() + viewportSize
            val knobPosition = (state.value.toFloat() / contentSize) * viewportSize + padding.toPx()
            val knobSize = fixedKnobRatio?.let { it * viewportSize } ?: ((viewportSize * viewportSize) / contentSize)
            val crossAxisPaddingPx = crossAxisPadding.toPx()
            ...
            drawRoundRect(
                ...
                topLeft =
                    when {
                        // When the scrollbar is horizontal and aligned to the bottom:
                        horizontal && alignEnd -> Offset(padding.toPx(), size.height - thickness.toPx() - crossAxisPaddingPx)
                        // When the scrollbar is horizontal and aligned to the top:
                        horizontal && !alignEnd -> Offset(padding.toPx(), crossAxisPaddingPx)
                        // When the scrollbar is vertical and aligned to the end:
                        alignEnd -> Offset(size.width - thickness.toPx() - crossAxisPaddingPx, padding.toPx())
                        // When the scrollbar is vertical and aligned to the start:
                        else -> Offset(crossAxisPaddingPx, padding.toPx())
                    },
                size =
                    if (horizontal) {
                        Size(size.width - padding.toPx() * 2, thickness.toPx())
            ...
            drawRoundRect(
                ...
                topLeft =
                    when {
                        // When the scrollbar is horizontal and aligned to the bottom:
                        horizontal && alignEnd -> Offset(knobPosition, size.height - thickness.toPx() - crossAxisPaddingPx)
                        // When the scrollbar is horizontal and aligned to the top:
                        horizontal && !alignEnd -> Offset(knobPosition, crossAxisPaddingPx)
                        // When the scrollbar is vertical and aligned to the end:
                        alignEnd -> Offset(size.width - thickness.toPx() - crossAxisPaddingPx, knobPosition)
                        // When the scrollbar is vertical and aligned to the start:
                        else -> Offset(crossAxisPaddingPx, knobPosition)
                    },
                size = if (horizontal) Size(knobSize, thickness.toPx()) else Size(
                    thickness.toPx(),
                    knobSize
                ),
                alpha = alpha,
